...
def test_cli_domain_params():
    ...
    assert "{ipc, variable_duration}" in result.stdout

def test_cli_instance_params():
    result = run_cli(["instance-params", "matchcellar", "-d", "version", "1", "-d", "variant", "ipc"])
    assert result.returncode == 0
    assert "Parameter" in result.stdout
...
def test_cli_mkinstance_stdout():
    ...
    result = run_cli([
        "mkinstance", "matchcellar",
        "-d", "version", "1", "-d", "variant", "ipc",
        "-p", "n_matches", "1", "-p", "n_fuses", "1",
        "--format", "pddl"
    ])
    assert result.returncode == 0
...
def test_cli_mkinstance_file(tmp_path):
    ...
    result = run_cli([
        ...
        "-d", "version", "1", "-d", "variant", "ipc",
        "-p", "n_matches", "1", "-p", "n_fuses", "1",
        "-D", str(dom_file), "-o", str(prob_file),
        "--format", "pddl"
    ])
    assert result.returncode == 0
