...
class TestMaJSPGenerator(unittest.TestCase):
    ...
    def test_parameter_spaces(self):
        ...
        domain_params = self.factory.parse_configuration({"max_robots": 2, "max_pallets": 2, "max_positions": 5}, dom_space)
    ...
    def test_instance_generation(self):
        ...
        inst_space = self.factory.get_instance_parameter_space(self.domain_name, domain_params)
        instance_params = self.factory.parse_configuration({"n_robots": 1, "n_pallets": 1, "n_positions": 3, "n_treatments": 2}, inst_space)

        problem = self.factory.generate_instance(self.domain_name, domain_params, instance_params)
        self.assertEqual(len(problem.goals), 2) # n_pallets * min(n_treatments, n_positions) = 1 * 2 = 2
