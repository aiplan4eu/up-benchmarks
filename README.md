...
ANML has no syntax for plan quality metrics, so a domain that defines one (for example `petri-net`) loses it when written as ANML. This only prints a warning: the problem is still written and remains solvable, it just carries no metric to optimise.
...
| petri-net | `from upbm.domains.petri_net import PetriNetGenerator` |
...
| **Petri Net** (IPC 2026) | UP/PDDL | Numbers, Plan Quality Metric | Tokens moved along hyper-edges of size up to 4; every action costs 1, so the metric is the plan length |
