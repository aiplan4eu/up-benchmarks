...
ANML has no syntax for plan quality metrics, so a domain that defines one (for example `ztalloc-sum`) loses it when written as ANML. This only prints a warning: the problem is still written and remains solvable, it just carries no metric to optimise.
